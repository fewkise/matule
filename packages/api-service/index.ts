...
import { ICategory, IOrder, IProduct, IProject, IProjectCategory, IAuthResponse, ICartItem } from "./types";
...
export const apiService = {
    ...
    registerUser: async (email: string, password: string, userData: any): Promise<IAuthResponse> => {
    ...
    syncCartItem: async (userId: string, productId: string | number, quantity: number): Promise<ICartItem> => {
    ...
    getProducts: async (search = '', categoryId = null): Promise<IProduct[]> => {
    ...
    getProjectCategories: async (): Promise<IProjectCategory[]> => {
        return request('/rest/v1/projectCategories?select=*', {
            headers: { 'Authorization': `Bearer ${supabaseKey}` },
        });
    },

    getProjects: async (userId: string, token: string): Promise<IProject[]> => {
        return request(`/rest/v1/projects?user_id=eq.${userId}&select=*`, {
            headers: { 'Authorization': `Bearer ${token}` },
        });
    },

    createProject: async (projectData: any, token: string): Promise<IProject> => {
    ...
    removeFromCartRequest: async (userId: string, productId: string | number): Promise<void> => {
        return request(`/rest/v1/cart?user_id=eq.${userId}&product_id=eq.${productId}`, {
            method: 'DELETE'
        });
    },

    uploadImage: async (uri: string, fileName: string, token: string): Promise<string> => {
